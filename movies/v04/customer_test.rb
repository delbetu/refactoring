...
class CustomerTest < Minitest::Test
  ...
  def test_regular_rental1_day
    ...
    movie = MovieBuilder.new.with_title(movie_name).with_price_code(Movie::REGULAR).build
  ...
  def test_newreleaserental1_day
    movie_name = 'movie_name'
    movie = MovieBuilder.new.with_title(movie_name).with_price_code(Movie::NEW_RELEASE).build
    rental = RentalBuilder.new(movie).with_days_rented(1).build
    customer_name = 'customer_name'
    customer = CustomerBuilder.new.with_name(customer_name).with_rental(rental).build
    statement = customer.statement
  ...
  def test_childrens_rental1_day
    movie_name = 'movie_name'
    movie = MovieBuilder.new.with_title(movie_name).with_price_code(Movie::CHILDRENS).build
    rental = RentalBuilder.new(movie).with_days_rented(1).build
    customer_name = 'customer_name'
    customer = CustomerBuilder.new.with_name(customer_name).with_rental(rental).build
    statement = customer.statement
  ...
  def test_rental
    ...
    regular_movie = MovieBuilder.new.with_title(regular_movie_name).with_price_code(Movie::REGULAR).build
    regular_rental = RentalBuilder.new(regular_movie).with_days_rented(10).build

    new_release_movie_name = 'new_release_movie_name'
    new_release_movie = MovieBuilder.new.with_title(new_release_movie_name).with_price_code(Movie::NEW_RELEASE).build
    ...
    childrens_movie = MovieBuilder.new.with_title(childrens_movie_name).with_price_code(Movie::CHILDRENS).build
